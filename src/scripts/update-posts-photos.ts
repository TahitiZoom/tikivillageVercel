/**
 * Script pour mettre à jour les posts existants avec les photos supplémentaires de Facebook
 *
 * Usage: npx tsx src/scripts/update-posts-photos.ts
 *
 * Ce script:
 * 1. Récupère tous les posts avec un facebookId
 * 2. Pour chaque post, récupère les attachments (photos) depuis Facebook
 * 3. Compare avec les images déjà dans le contenu
 * 4. Télécharge et ajoute les photos manquantes
 */

import 'dotenv/config'
import { getPayload } from 'payload'
import config from '@payload-config'

interface FacebookAttachment {
  media?: { image?: { src: string } }
  type?: string
  url?: string
  subattachments?: {
    data: Array<{
      media?: { image?: { src: string } }
      type?: string
      url?: string
    }>
  }
}

interface FacebookAttachmentsResponse {
  data: FacebookAttachment[]
  error?: { message: string }
}

interface LexicalNode {
  [x: string]: unknown
  type: string
  children?: LexicalNode[]
  fields?: {
    blockType?: string
    media?: number | { id: number }
    images?: Array<{ image: number | { id: number } }>
  }
}

interface LexicalContent {
  [x: string]: unknown
  root: {
    children: LexicalNode[]
    direction: 'ltr' | 'rtl' | null
    format: '' | 'left' | 'start' | 'center' | 'right' | 'end' | 'justify'
    indent: number
    type: string
    version: number
  }
}

// Download image from URL
async function downloadImage(url: string): Promise<{ buffer: Buffer; contentType: string } | null> {
  try {
    const response = await fetch(url, { redirect: 'follow' })
    if (!response.ok) return null

    const contentType = response.headers.get('content-type') || 'image/jpeg'
    const arrayBuffer = await response.arrayBuffer()
    const buffer = Buffer.from(arrayBuffer)

    return { buffer, contentType }
  } catch {
    return null
  }
}

// Fetch all photo attachments for a Facebook post
async function fetchPostAttachments(postId: string, accessToken: string): Promise<string[]> {
  try {
    const url = `https://graph.facebook.com/v22.0/${postId}/attachments?fields=subattachments{media,type,url},media,type,url&access_token=${accessToken}`
    const response = await fetch(url)
    if (!response.ok) return []

    const data: FacebookAttachmentsResponse = await response.json()
    if (data.error || !data.data) return []

    const imageUrls: string[] = []

    for (const attachment of data.data) {
      // Check for subattachments (multiple photos)
      if (attachment.subattachments?.data) {
        for (const sub of attachment.subattachments.data) {
          if (sub.type === 'photo' && sub.media?.image?.src) {
            imageUrls.push(sub.media.image.src)
          }
        }
      }
      // Single attachment
      else if (attachment.type === 'photo' && attachment.media?.image?.src) {
        imageUrls.push(attachment.media.image.src)
      }
    }

    return imageUrls
  } catch {
    return []
  }
}

// Extract existing media IDs from Lexical content
function getExistingMediaIds(content: LexicalContent | null): number[] {
  if (!content?.root?.children) return []

  const mediaIds: number[] = []

  function walkNodes(nodes: LexicalNode[]) {
    for (const node of nodes) {
      if (node.type === 'block' && node.fields) {
        // MediaBlock
        if (node.fields.blockType === 'mediaBlock' && node.fields.media) {
          const mediaId = typeof node.fields.media === 'number'
            ? node.fields.media
            : node.fields.media.id
          if (mediaId) mediaIds.push(mediaId)
        }
        // GalleryBlock (if it exists)
        if (node.fields.blockType === 'galleryBlock' && node.fields.images) {
          for (const img of node.fields.images) {
            const mediaId = typeof img.image === 'number' ? img.image : img.image?.id
            if (mediaId) mediaIds.push(mediaId)
          }
        }
      }
      if (node.children) {
        walkNodes(node.children)
      }
    }
  }

  walkNodes(content.root.children)
  return mediaIds
}

// Add MediaBlock nodes to Lexical content
function addMediaBlocksToContent(content: LexicalContent, mediaIds: number[]): LexicalContent {
  const newChildren = [...content.root.children]

  // Find the signature paragraph (usually last text paragraph)
  let insertIndex = newChildren.length
  for (let i = newChildren.length - 1; i >= 0; i--) {
    const node = newChildren[i]
    if (node.type === 'paragraph' && node.children) {
      const textNode = node.children.find((c: LexicalNode & { text?: string }) => c.type === 'text')
      if (textNode && 'text' in textNode && typeof textNode.text === 'string' && textNode.text.includes('Stephane Sayeb')) {
        insertIndex = i + 1
        break
      }
    }
  }

  // Create MediaBlock nodes
  const mediaBlocks: LexicalNode[] = mediaIds.map(mediaId => ({
    type: 'block',
    fields: {
      blockType: 'mediaBlock',
      media: mediaId,
    },
  }))

  // Insert at the correct position
  newChildren.splice(insertIndex, 0, ...mediaBlocks)

  return {
    root: {
      ...content.root,
      children: newChildren,
    },
  }
}

// Main script
async function main() {
  console.log('===========================================')
  console.log('Mise a jour des photos des posts Facebook')
  console.log('===========================================\n')

  // Check environment variables
  const pageAccessToken = process.env.FB_PAGE_ACCESS_TOKEN || process.env.FACEBOOK_PAGE_ACCESS_TOKEN
  const pageId = process.env.FB_PAGE_ID || process.env.FACEBOOK_PAGE_ID

  if (!pageAccessToken || !pageId) {
    console.error('Erreur: Variables d\'environnement manquantes')
    console.error('  - FB_PAGE_ACCESS_TOKEN ou FACEBOOK_PAGE_ACCESS_TOKEN')
    console.error('  - FB_PAGE_ID ou FACEBOOK_PAGE_ID')
    process.exit(1)
  }

  const payload = await getPayload({ config })

  // Get all posts with a facebookId
  console.log('Recherche des posts avec un facebookId...')

  const postsResult = await payload.find({
    collection: 'posts',
    where: {
      facebookId: {
        exists: true,
      },
    },
    limit: 1000,
    depth: 1, // Load coverImage relation
  })

  console.log(`${postsResult.docs.length} posts trouves avec un facebookId\n`)

  if (postsResult.docs.length === 0) {
    console.log('Aucun post a mettre a jour.')
    process.exit(0)
  }

  // Stats
  let postsUpdated = 0
  let photosAdded = 0
  let errorsCount = 0
  const errors: string[] = []

  // Process each post
  for (let i = 0; i < postsResult.docs.length; i++) {
    const post = postsResult.docs[i]
    const progress = `[${i + 1}/${postsResult.docs.length}]`
    const postTitle = post.title?.slice(0, 50) || 'Sans titre'

    console.log(`${progress} ${postTitle}...`)

    if (!post.facebookId) {
      console.log(`  -> Pas de facebookId, ignore`)
      continue
    }

    try {
      // Fetch attachments from Facebook
      const allImageUrls = await fetchPostAttachments(post.facebookId, pageAccessToken)

      if (allImageUrls.length === 0) {
        console.log(`  -> Aucune photo trouvee sur Facebook`)
        continue
      }

      console.log(`  -> ${allImageUrls.length} photo(s) trouvee(s) sur Facebook`)

      // Skip the first image (already coverImage)
      const additionalUrls = allImageUrls.slice(1)

      if (additionalUrls.length === 0) {
        console.log(`  -> Pas de photos supplementaires`)
        continue
      }

      // Get existing media IDs in the content
      const existingMediaIds = getExistingMediaIds(post.content as LexicalContent | null)
      console.log(`  -> ${existingMediaIds.length} image(s) deja dans le contenu`)

      // Calculate how many new images to add
      // If the post has N images on Facebook and we already have (N-1) in content, skip
      if (existingMediaIds.length >= additionalUrls.length) {
        console.log(`  -> Deja a jour`)
        continue
      }

      // Download and create new media
      const newMediaIds: number[] = []
      const imagesToAdd = additionalUrls.length - existingMediaIds.length

      console.log(`  -> Telechargement de ${imagesToAdd} nouvelle(s) image(s)...`)

      for (let imgIdx = existingMediaIds.length; imgIdx < additionalUrls.length; imgIdx++) {
        const imgUrl = additionalUrls[imgIdx]

        try {
          const imageData = await downloadImage(imgUrl)
          if (imageData) {
            const filename = `facebook-${post.facebookId}-${imgIdx + 1}.jpg`
            const mediaDoc = await payload.create({
              collection: 'media',
              data: {
                alt: `${postTitle} - Photo ${imgIdx + 2}`,
              },
              file: {
                data: imageData.buffer,
                mimetype: imageData.contentType,
                name: filename,
                size: imageData.buffer.length,
              },
            })
            newMediaIds.push(mediaDoc.id)
            photosAdded++
          }
        } catch (imgError) {
          console.log(`    ! Erreur image ${imgIdx + 1}: ${imgError instanceof Error ? imgError.message : 'Erreur inconnue'}`)
        }
      }

      if (newMediaIds.length === 0) {
        console.log(`  -> Aucune nouvelle image ajoutee`)
        continue
      }

      // Update the post content with new MediaBlocks
      const currentContent = post.content as LexicalContent | null
      if (!currentContent?.root) {
        console.log(`  -> Contenu invalide, ignore`)
        continue
      }

      const updatedContent = addMediaBlocksToContent(currentContent, newMediaIds)

      await payload.update({
        collection: 'posts',
        id: post.id,
        data: {
          content: updatedContent,
        },
      })

      postsUpdated++
      console.log(`  -> ${newMediaIds.length} photo(s) ajoutee(s)`)

    } catch (postError) {
      errorsCount++
      const errorMsg = `Erreur post ${post.facebookId}: ${postError instanceof Error ? postError.message : 'Erreur inconnue'}`
      errors.push(errorMsg)
      console.log(`  ! ${errorMsg}`)
    }
  }

  // Summary
  console.log('\n===========================================')
  console.log('Resume')
  console.log('===========================================')
  console.log(`Posts mis a jour: ${postsUpdated}`)
  console.log(`Photos ajoutees:  ${photosAdded}`)
  console.log(`Erreurs:          ${errorsCount}`)

  if (errors.length > 0) {
    console.log('\nDetails des erreurs:')
    for (const err of errors) {
      console.log(`  - ${err}`)
    }
  }

  console.log('\nTermine.')
  process.exit(0)
}

main().catch((error) => {
  console.error('Erreur fatale:', error)
  process.exit(1)
})
