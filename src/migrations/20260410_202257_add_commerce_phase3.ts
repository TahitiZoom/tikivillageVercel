...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`products_booking_available_days\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`products_booking_available_days_order_idx\` ON \`products_booking_available_days\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`products_booking_available_days_parent_idx\` ON \`products_booking_available_days\` (\`parent_id\`);`)
  await db.run(sql`CREATE TABLE \`products_booking_blocked_dates\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`products_booking_blocked_dates_order_idx\` ON \`products_booking_blocked_dates\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`products_booking_blocked_dates_parent_id_idx\` ON \`products_booking_blocked_dates\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`products_wedding_options\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`price\` numeric,
  ...
  `)
  await db.run(sql`CREATE INDEX \`products_wedding_options_order_idx\` ON \`products_wedding_options\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`products_wedding_options_parent_id_idx\` ON \`products_wedding_options\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`products_wedding_options_locales\` (
  	\`name\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`products_wedding_options_locales_locale_parent_id_unique\` ON \`products_wedding_options_locales\` (\`_locale\`,\`_parent_id\`);`)
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`bookings_booking_reference_idx\` ON \`bookings\` (\`booking_reference\`);`)
  await db.run(sql`CREATE INDEX \`bookings_product_idx\` ON \`bookings\` (\`product_id\`);`)
  await db.run(sql`CREATE INDEX \`bookings_customer_idx\` ON \`bookings\` (\`customer_id\`);`)
  await db.run(sql`CREATE INDEX \`bookings_order_idx\` ON \`bookings\` (\`order_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`orders_booking_idx\` ON \`orders\` (\`booking_id\`);`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_pages\`("id", "published_at", "generate_slug", "slug", "updated_at", "created_at", "_status") SELECT "id", "published_at", "generate_slug", "slug", "updated_at", "created_at", "_status" FROM \`pages\`;`)
  ...
  await db.run(sql`DROP INDEX \`pages_meta_meta_image_idx\`;`)
  await db.run(sql`ALTER TABLE \`pages_locales\` ADD \`title\` text;`)
  await db.run(sql`ALTER TABLE \`pages_locales\` ADD \`hero_type\` text DEFAULT 'lowImpact';`)
  await db.run(sql`ALTER TABLE \`pages_locales\` ADD \`hero_rich_text\` text;`)
  await db.run(sql`ALTER TABLE \`pages_locales\` ADD \`hero_media_id\` integer REFERENCES media(id);`)
  ...
  await db.run(sql`DROP INDEX \`_pages_v_version_meta_version_meta_image_idx\`;`)
  await db.run(sql`ALTER TABLE \`_pages_v_locales\` ADD \`version_title\` text;`)
  await db.run(sql`ALTER TABLE \`_pages_v_locales\` ADD \`version_hero_type\` text DEFAULT 'lowImpact';`)
  await db.run(sql`ALTER TABLE \`_pages_v_locales\` ADD \`version_hero_rich_text\` text;`)
  await db.run(sql`ALTER TABLE \`_pages_v_locales\` ADD \`version_hero_media_id\` integer REFERENCES media(id);`)
  ...
  await db.run(sql`ALTER TABLE \`users\` ADD \`role\` text DEFAULT 'customer';`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`products_booking_available_days\`;`)
  await db.run(sql`DROP TABLE \`products_booking_blocked_dates\`;`)
  await db.run(sql`DROP TABLE \`products_wedding_options\`;`)
  await db.run(sql`DROP TABLE \`products_wedding_options_locales\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_pages_locales\`("meta_title", "meta_image_id", "meta_description", "id", "_locale", "_parent_id") SELECT "meta_title", "meta_image_id", "meta_description", "id", "_locale", "_parent_id" FROM \`pages_locales\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new__pages_v_locales\`("version_meta_title", "version_meta_image_id", "version_meta_description", "id", "_locale", "_parent_id") SELECT "version_meta_title", "version_meta_image_id", "version_meta_description", "id", "_locale", "_parent_id" FROM \`_pages_v_locales\`;`)
